...
def cb_imu_calibrate(rpy_data):
    ...
    # rotate around body-fix frame y for 180 degree, post-multiply
    R_filter = np.array([[cos(pi), 0, sin(pi)],
                         [0, 1, 0],
                         [-sin(pi), 0, cos(pi)]])

    R_calibrated = R.dot(R_filter)
    ...
    rpy_c.vector.y = y_c
    rpy_c.vector.z = z_c
    rpy_c.header = rpy_data.header  # fill the header with the same value?
